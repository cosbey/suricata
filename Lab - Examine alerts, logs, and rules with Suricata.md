...
_**Note:** The terms **rules** and **signatures** are used interchangeably in this lab activity._
...
_**Note:** The `less` command can also be used to read file content one page at a time, making it useful for reading lengthy output._
...
_**Note:** Rule order refers to the order in which rules are evaluated by Suricata. Rules are loaded in the order in which they are defined in the configuration file. However, Suricata processes rules in a different default order: pass, drop, reject, and alert. Rule order affects the final verdict of a packet._
...
![[Pasted image 20230827171833.png]]
...
![[Pasted image 20230827172242.png]]
...
![[Pasted image 20230827172642.png]]
...
`
![[Pasted image 20230827172859.png]]
...
_**Note:** _You can use the lowercase **f** and **b** keys to move forward or backward through the output. Also, if you enter a command incorrectly and it fails to return to the command-line prompt, you can press **CTRL+C** to stop the process and force the shell to return to the command-line prompt.__
...
_**Note:** _The `jq` tool is very useful for processing JSON data, however, a full explanation of its capabilities is outside of the scope of this lab.__
...
![[Pasted image 20230827173230.png]]
...
_**Note:** _The `jq` command above extracts the fields specified in the list in the square brackets from the JSON payload. The fields selected are the timestamp (`.timestamp`), the flow id (`.flow_id`), the alert signature or msg (`.alert.signature`), the protocol (`.proto`), and the destination IP address (`.dest_ip`).__

![[Pasted image 20230827173836.png]]
